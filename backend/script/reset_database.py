...
with Session(engine) as session:
    ...
    to_entity = ClubEntity.from_model
    session.add_all([to_entity(model) for model in clubs.models])
    session.execute(text(f'ALTER SEQUENCE {ClubEntity.__table__}_id_seq RESTART WITH {len(clubs.models) + 1}'))
    session.commit()

# Add Events
with Session(engine) as session:
    from .dev_data import events
    to_entity = EventEntity.from_model
    session.add_all([to_entity(model) for model in events.models])
    session.execute(text(f'ALTER SEQUENCE {EventEntity.__table__}_id_seq RESTART WITH {len(events.models) + 1}'))
    session.commit()